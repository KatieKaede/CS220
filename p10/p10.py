...
spectral_type = stars_rows[row_idx][stars_header.index('Spectral Type')]
stellar_effective_temperature = float(stars_rows[row_idx][stars_header.index('Stellar Effective Temperature [K]')])
stellar_radius = float(stars_rows[row_idx][stars_header.index('Stellar Radius [Solar Radius]')])
stellar_mass = float(stars_rows[row_idx][stars_header.index('Stellar Mass [Solar mass]')])
stellar_luminosity = float(stars_rows[row_idx][stars_header.index('Stellar Luminosity [log(Solar)]')])
stellar_surface_gravity = float(stars_rows[row_idx][stars_header.index('Stellar Surface Gravity [log10(cm/s**2)]')])
stellar_age = float(stars_rows[row_idx][stars_header.index('Stellar Age [Gyr]')])
...
def get_stars(star_file):
    ...
    read_file_data = process_csv(os.path.join('data', star_file))
    ...
    # TODO: retubrn the dictionary at the end of the loop
...
for star in stars_1_dict:
    if stars_1_dict[star].stellar_luminosity == None:
        ...
        star_sum += stars_1_dict[star].stellar_luminosity
        star_count += 1
        
avg_lum_stars_1 = star_sum / star_count
avg_lum_stars_1
# -
grader.check("q7")
...
for star in stars_2_dict:
    if stars_2_dict[star].stellar_age == None:
        continue
        
    else:
        stars2_sum += stars_2_dict[star].stellar_age
        stars2_count += 1

avg_age_stars_2 = stars2_sum / stars2_count
avg_age_stars_2
...
for row in stars_paths_updated:
     stars_dict.update(get_stars(row))
...
for star in stars_dict:
    ...
    if stars_dict[star].stellar_radius > large_star_val:
        large_star_val = stars_dict[star].stellar_radius
        biggest_star = star
...
for star in stars_dict:
    if star[0:6] == 'Kepler':
        if stars_dict[star].stellar_age == None:
            continue
        kepler_sum += stars_dict[star].stellar_age
        kepler_count += 1
        
avg_age_kepler = kepler_sum / kepler_count
avg_age_kepler  
# -
...
planets_5_data = process_csv(os.path.join("data", "planets_1.csv"))
planets_5_rows = planets_5_data[1:]

# extract the values from planets_1_rows
planet_name = str(planet_cell(row_idx, 'Planet Name', planets_5_rows))
host_name = mapping_1_json[planet_name]
discovery_method = planet_cell(row_idx, 'Discovery Method', planets_5_rows)
discovery_year = planet_cell(row_idx, 'Discovery Year', planets_5_rows)
controversial_flag = planet_cell(row_idx, 'Controversial Flag', planets_5_rows)
orbital_period = planet_cell(row_idx, 'Orbital Period [days]', planets_5_rows)
planet_radius = planet_cell(row_idx, 'Planet Radius [Earth Radius]', planets_5_rows)
planet_mass = planet_cell(row_idx, 'Planet Mass [Earth Mass]', planets_5_rows)
semi_major_radius = planet_cell(row_idx, 'Orbit Semi-Major Axis [au]', planets_5_rows)
eccentricity = planet_cell(row_idx, 'Eccentricity', planets_5_rows)
equilibrium_temperature = planet_cell(row_idx, 'Equilibrium Temperature [K]', planets_5_rows)
insolation_flux = planet_cell(row_idx, 'Insolation Flux [Earth Flux]', planets_5_rows) # extract the value in column 'Insolation Flux [Earth Flux]' using 'planet_cell'
...
def get_planets(planet_file, mapping_file):
    ...
    current_planet_file = process_csv(os.path.join('data', planet_file))
    file_header = current_planet_file[0]
    file_rows = current_planet_file[1:]
    try:
        json_mapping = read_json(os.path.join('data', mapping_file))
...
last_five_planets_4_list = []
for planet in get_planets('planets_4.csv', 'mapping_4.json'):
    last_five_planets_4_list.append(planet)
    
last_five_planets_4 = last_five_planets_4_list[-5:]
last_five_planets_4
# -
grader.check("q15")
...
for idx in range(len(planets_paths)):
    planets_list.extend(get_planets(planets_paths[idx], json_paths[idx]))
...
for planet_idx in planets_list:
    if planet_idx.planet_name == 'TOI-2202 c':
        toi_2022_c_star = stars_dict[planet_idx.host_name]
...
planets_sum = 0
planets_count = 0
for special_planet in planets_list:
    if stars_dict[special_planet.host_name].stellar_radius == None:
        continue
    elif stars_dict[special_planet.host_name].stellar_radius > 10:
        
        if special_planet.planet_radius == None:
            continue
        else:
            planets_sum += special_planet.planet_radius
            planets_count += 1
            
avg_planet_radius_big_stars = planets_sum / planets_count
...
stars_dict['Kepler-1663']

# +
# compute and store the answer in the variable 'youngest_star_planets', then display it
youngest_star_planets = []
min_age = None
for young_stars in stars_dict:
    if stars_dict[young_stars].stellar_age == None:
        continue
        
    elif min_age is None or stars_dict[young_stars].stellar_age < min_age:
        min_age = stars_dict[young_stars].stellar_age
        
for orbiting_planets in planets_list:
    if stars_dict[orbiting_planets.host_name].stellar_age == min_age:
        youngest_star_planets.append(orbiting_planets)
