...
nm_values = []
for code in small_mapping:
    if code[0:2] == 'nm':
        nm_values.append(small_mapping[code])
...
for code in small_mapping:
    ...
    lastName = small_mapping[code].split()[-1]
    
    if lastName == 'Spencer' and code[0:2] == 'nm':
        nm_spencer.append(code)
...
real_length = copy.copy((movies))
hepburn_films = find_specific_movies(real_length, "Katharine Hepburn")
hepburn_films
...
actual_length = copy.copy((movies))
wisconsin_movies = find_specific_movies(actual_length, "Wisconsin")
wisconsin_movies
...
for info in genre_dict["Horror"]:
    ...
    if info['rating'] > 9.0:
        horror_movies_above_9.append(info['title'])
...
movies_real_length = copy.copy(movies)
jennifer_dict = {}

jen_films = find_specific_movies(movies_real_length, "Jennifer Aniston")

jen_films_organized = bucketize_by_genre(jen_films)

highest_genre_number = 0
for jen_genre in jen_films_organized:
    if len(jen_films_organized[jen_genre]) > highest_genre_number:
        highest_genre_number = len(jen_films_organized[jen_genre])
        jen_aniston_genre = jen_genre
...
base_rate = 0
documentary_movies = copy.copy(genre_dict["Documentary"])

for info in documentary_movies:
    highest_rating = info['rating']
    if highest_rating > base_rate:
        base_rate = highest_rating
        highest_score = copy.copy(base_rate)
        
for info in documentary_movies:
        
    if info['rating'] == highest_score:

        if str(info['directors']) in max_docu_rating_directors:
            continue
        else:
            max_docu_rating_directors.append(' '.join(info['directors']))
