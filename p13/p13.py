...
# I DELETED SOME EXAMPLE GRAPHS/DATAFRAMES BECAUSE JUPYTER SAID MY FILE WAS TOO BIG FOR GRADESCOPE
...
def bar_plot(df, x, y):
    ...
    df_index = df.set_index(x)
    plotted_graph = df_index.plot.bar(legend=False, color='0')
    plotted_graph.set_xlabel(x)
    plotted_graph.set_ylabel(y)
...
def scatter_plot(df, x, y):
    ...
    plotted_scatter = df.plot.scatter(x=x, y=y, c='black')
...
def regression_line_plot(df, x, y):
    ...
    regress_coeff = get_regression_coeff(df, x, y)
    m = regress_coeff[0]
    b = regress_coeff[1]
    ...
    plot_line_graph = df.plot.line(ax=ax, x=x, y='fit', color="red")
    
    return plot_line_graph
...
rankings = rankings = pd.read_sql("SELECT * FROM rankings", conn)
...
others_than_10 = num_institutions.iloc[10:]
other_sum = sum(others_than_10['num_of_institutions'])

num_institutions = top_10_countries.append({"country": "Other", "num_of_institutions": other_sum}, ignore_index=True)
...
# then create the scatter plot using the DataFrame
top_ranked_inter_faculty = qry("""
SELECT `country`, MIN(`rank`), `international_students`, `faculty_student_score`
FROM rankings
WHERE `year` = 2020
GROUP BY `country`
""")
...
uk_inter_score_corr = corr_uk.loc['overall_score'][0]
uk_inter_score_corr
...
citations_overall_df = qry("""
SELECT `overall_score`, (`citations_per_faculty` * `international_faculty`) / 100 AS citations_per_international
FROM rankings
WHERE `year` = 2019
""")

corr_df = citations_overall_df.corr()
cit_per_inter_score_corr = corr_df.loc["citations_per_international"][0]
...
avg_cit_per_inter = qry("""
SELECT `country`, AVG((`citations_per_faculty` * `international_faculty`) / 100) avg_inter_citations
FROM rankings
...
max_cit_per_inter = qry("""
SELECT `country`, `institution_name`, MAX((`citations_per_faculty` * `international_faculty`) / 100) max_inter_citations
FROM rankings
WHERE `year` = 2020
GROUP BY `country`
HAVING `max_inter_citations` IS NOT NULL
...
coeffs = get_regression_coeff(inverse_overall_rank, "inverse_overall_score", "rank")
rank_score_72 = math.ceil(coeffs[0] * (1/72) + coeffs[1])
rank_score_72
