...
# project: p12
# submitter: 908 323 7793, Katie Krause
# partner: None
...
countries = pd.Series(rankings['Country'])
...
uw_madison_year = uw_madison[uw_madison['Year'] == "2021-2022"]
uw_madison_nat_rank = uw_madison_year.iloc[0]["National Rank"]
uw_madison_nat_rank
...
germany_parameters = rankings[(rankings['Country'] == 'Germany') & (rankings['Year'] == '2019-2020') &\
                             (rankings['National Rank'] == 1)]
german_best_name = germany_parameters['Institution'].iloc[0]
german_best_name
...
germany_cell = germany_parameters
germany_world_rank = germany_cell['World Rank'].iloc[0]

usa_dataframe = rankings[(rankings['Country'] == "USA") & (rankings['Year'] == '2019-2020')]
usa_ranking = usa_dataframe['World Rank'] < germany_world_rank
us_better_than_german_best = (usa_dataframe[usa_ranking]['Institution']).tolist()
...
china_requirements = rankings[(rankings['Year'] == "2021-2022") & (rankings['Country'] == 'China')]
china_highest_qoe_number = (pd.Series.min(china_requirements['Quality of Education Rank']))
china_highest_qoe_inst = china_requirements['Institution'][china_requirements['Quality of Education Rank'] == china_highest_qoe_number]
china_highest_qoe = china_highest_qoe_inst.iloc[0]
china_highest_qoe
...
india_requirements = rankings[(rankings['Country'] == "India") & (rankings['Year'] == '2020-2021')]
india_sorted_research = india_requirements.sort_values(by='Research Performance Rank')
india_highest_research_sliced = india_sorted_research['Institution'].iloc[:5]
india_highest_research = india_highest_research_sliced.tolist()
...
institutions_df_list = []
for school in institutions_2019_2020_2021:
    data = {}
    
    data['Institution'] = school
    
    true_dataframe = rankings[rankings['Institution'] == school]
    data['2019_ranking'] = (true_dataframe[true_dataframe['Year'] == '2019-2020']['World Rank']).iloc[0]
    data['2020_ranking'] = (true_dataframe[true_dataframe['Year'] == '2020-2021']['World Rank']).iloc[0]
    data['2021_ranking'] = (true_dataframe[true_dataframe['Year'] == '2021-2022']['World Rank']).iloc[0]
    institutions_df_list.append(data)
    
institutions_df = DataFrame(institutions_df_list)
institutions_df
...
over_500_inst = institutions_df[institutions_df["difference"] >= 500]
improved_institutions = over_500_inst["Institution"].tolist()
...
highest_diff = pd.Series.max(institutions_df["difference"])
institution_cell = institutions_df[institutions_df["difference"] == highest_diff]
most_change_inst = institution_cell["Institution"].iloc[0]
most_change_inst
...
institutions_df['2019-2020'] = institutions_df['2019_ranking'] - institutions_df['2020_ranking']
institutions_df['2020-2021'] = institutions_df['2020_ranking'] - institutions_df['2021_ranking']

improved_parameters = institutions_df[(institutions_df['2019-2020'] > 0) & (institutions_df['2020-2021'] > 0)]
strictly_improved = len(improved_parameters)
strictly_improved
...
alumni_df = year_2020_ranking_df.sort_values(by="Alumni Employment Rank")
alumni_df_top_ten = alumni_df[:10]

top_world_rankings = rankings[rankings['World Rank'] <= 10]

top_only_aer = list(set(alumni_df_top_ten['Institution']) - set(top_world_rankings['Institution']))
top_only_aer
...
top_in_2019 = year_2019_ranking_df[year_2019_ranking_df['World Rank'] <= 100 ]
top_in_2020 = year_2021_ranking_df[year_2021_ranking_df['World Rank'] <= 100 ]

top_only_2019 = list(set(top_in_2019['Institution']) - set(top_in_2020['Institution']))
top_only_2019
...
top_2020_rank = rankings[(rankings['Year'] == "2020-2021") & (rankings['World Rank'] <= 100)]
value_counts = top_2020_rank['Country'].value_counts()
more_than_10 = value_counts[value_counts >= 10]
top_countries = list(more_than_10.index)
top_countries
...
bs_doc = BeautifulSoup(f.read(), "html.parser")
bs_table = bs_doc.find('bs_table')
all_columns = bs_doc.find_all("th")
header = [name.get_text() for name in all_columns]
f.close()
...
def parse_html(html):
    ...
    for row in table.find_all('tr')[1:]:
        ...
        dict_of_rows = {}
        dict_of_rows['Year'] = html.split('.')[0]
        for idx in range(len(header)):
            if values[idx].text == '-':
                dict_of_rows[header[idx]] = None
                
            elif idx in [0,3,4,5,6,7]:
                dict_of_rows[header[idx]] = int(values[idx].text)
                
            elif idx in [8]:
                dict_of_rows[header[idx]] = float(values[idx].text)
                
            else:
                dict_of_rows[header[idx]] = (values[idx].text)
                
        dict_of_tables.append(dict_of_rows)
...
parsed_19 = parse_html('2019-2020.html')
parsed_20 = parse_html('2020-2021.html')
parsed_21 = parse_html('2021-2022.html')

combined_file = []
for info in parsed_19:
    combined_file.append(info)
    
for info in parsed_20:
    combined_file.append(info)
    
for info in parsed_21:
    combined_file.append(info)
    

write_json("my_rankings.json", combined_file)
