...
def get_stars(star_file):
    ...
    read_file_data = process_csv(star_file)
    current_header = read_file_data[0]
    current_row = read_file_data[1:]
    
    all_stars_dict = {}
    for row_idx in range(len(current_row)):
        star_name = star_cell(row_idx, 'Name', current_row)
        spectral_type = star_cell(row_idx, 'Spectral Type', current_row)
        stellar_effective_temperature = star_cell(row_idx, 'Stellar Effective Temperature [K]', current_row)
        stellar_radius = star_cell(row_idx, 'Stellar Radius [Solar Radius]', current_row)
        stellar_mass = star_cell(row_idx, 'Stellar Mass [Solar mass]', current_row)
        stellar_luminosity = star_cell(row_idx, 'Stellar Luminosity [log(Solar)]', current_row)
        stellar_surface_gravity = star_cell(row_idx, 'Stellar Surface Gravity [log10(cm/s**2)]', current_row)
        stellar_age = star_cell(row_idx, 'Stellar Age [Gyr]', current_row)
        ...
        all_stars_dict[star_name] = star
    return all_stars_dict
...
for file in files_in_data:
    if file[0:5] == 'stars':
        stars_paths_updated.append(file)
        
stars_dict = {}
for row in stars_paths_updated:
     stars_dict.update(get_stars(os.path.join('data', row)))
...
def planet_cell(row_idx, col_name, planets_rows, header=planets_header):
    ...
    elif col_name in ['Planet Name'] or col_name in ['Discovery Method']:
        return str(val)
    
    elif col_name in ['Discover Year']:
        return int(val)
...
def get_planets(planet_file, mapping_file):
    ...
    current_planet_file = process_csv(planet_file)
    file_header = current_planet_file[0]
    file_rows = current_planet_file[1:]
    try:
        json_mapping = read_json(mapping_file)
    ...
    for row_idx in range(len(file_rows)):
        try:
            planet_name = str(planet_cell(row_idx, 'Planet Name', file_rows))
            host_name = json_mapping[planet_name]
            discovery_method = planet_cell(row_idx, 'Discovery Method', file_rows)
            discovery_year = planet_cell(row_idx, 'Discovery Year', file_rows)
            controversial_flag = planet_cell(row_idx, 'Controversial Flag', file_rows)
            orbital_period = planet_cell(row_idx, 'Orbital Period [days]', file_rows)
            planet_radius = planet_cell(row_idx, 'Planet Radius [Earth Radius]', file_rows)
            planet_mass = planet_cell(row_idx, 'Planet Mass [Earth Mass]', file_rows)
            semi_major_radius = planet_cell(row_idx, 'Orbit Semi-Major Axis [au]', file_rows)
            eccentricity = planet_cell(row_idx, 'Eccentricity', file_rows)
            equilibrium_temperature = planet_cell(row_idx, 'Equilibrium Temperature [K]', file_rows)
            insolation_flux = planet_cell(row_idx, 'Insolation Flux [Earth Flux]', file_rows)
...
for idx in range(len(planets_paths)):
    planets_list.extend(get_planets(os.path.join('data', planets_paths[idx]), os.path.join('data', json_paths[idx])))
...
stellar_mass_prediction = 133408 / statistics.mean(ratios_gj9827)
actual_mass = stars_dict['GJ 9827'].stellar_mass
percentage_change = ((stellar_mass_prediction - actual_mass) / actual_mass) * 100
percentage_change
...
for star in stars_dict:
    current_mass = stars_dict[star].stellar_mass
    
    if current_mass == None or current_mass < 0.3:
        continue
    
    elif current_mass >= 0.3 and current_mass < 8:
        star_classes['Red Giant'].append(star)
        
    elif current_mass >= 8 and current_mass < 10.5:
...
star_classes_avg_lum = {'Red Giant': [], 'White Dwarf': [], 'Neutron Star': []}
for star_class in star_classes:
    for star in star_classes[star_class]:
        star_luminos = stars_dict[star].stellar_luminosity
        if star_luminos == None:
            continue
            
        else:
            star_classes_avg_lum[star_class].append(star_luminos)
            
for key in star_classes_avg_lum:
    real_mean = statistics.mean(star_classes_avg_lum[key])
    star_classes_avg_lum[key] = real_mean  
...
for star_class in star_classes:
    ...
    lum_dict[star_class] = []
    for star in star_classes[star_class]:
        current_star_info = stars_dict[star]
        if current_star_info.stellar_mass == None or current_star_info.stellar_radius == None or current_star_info.stellar_luminosity == None:
            continue
        else:
            lum_dict[star_class].append(current_star_info.stellar_luminosity)
            density_dict[star_class].append(current_star_info.stellar_mass / (current_star_info.stellar_radius) ** 3)
...
for star_class in star_classes:
    ...
    for star in star_classes[star_class]:
        ...
        if (current_star_info.stellar_mass / (current_star_info.stellar_radius) ** 3) > 25:
            continue
            
        else:
            density_dict_no_outliers[star_class].append(current_star_info.stellar_mass / (current_star_info.stellar_radius) ** 3)
            lum_dict_no_outliers[star_class].append(current_star_info.stellar_luminosity)
...
for star_name in stars_dict:
    current_star = stars_dict[star_name]
    
    if current_star.stellar_effective_temperature == None or current_star.stellar_luminosity == None or \
    current_star.stellar_age == None or current_star.stellar_mass == None:
        continue
        
    else:
        temp_list.append(current_star.stellar_effective_temperature)
        lum_list.append(current_star.stellar_luminosity)
        age_list.append(current_star.stellar_age)
        mass_list.append(current_star.stellar_mass)
        

plot_scatter(temp_list, lum_list, "Effective Temperature", "Luminosity", s=mass_list, c=age_list)
# -
...
def get_all_paths_in(directory):
    ...
    for item in all_files:
        ...
        if os.path.isfile(os.path.join(directory, item)) == True:
            item_list.append((os.path.join(directory, item)))
            
        if os.path.isdir(os.path.join(directory, item)) == True:
            item_list.extend(get_all_paths_in(os.path.join(directory, item)))
...
broken_data_keplers_kepler_10s_others = get_all_paths_in(os.path.join \
                                                         ("broken_data", "keplers", "kepler_10s", "others"))
...
for file in broken_data:
    planet_obj = get_planets(os.path.join('data', 'planets_5.csv'), file)
    all_planets_list.extend(planet_obj)
...
def get_surface_gravity(planet):
    ...
    if planet.planet_mass == None or planet.planet_radius == None:
        ...
        surface_gf = planet.planet_mass / planet.planet_radius ** 2
...
for planet_info in all_planets_list:
    if planet_info.planet_name == 'GJ 674 b':
        gj_674_b_gravity = get_surface_gravity(planet_info)
        break
...
def get_distances_to_star(planet):
    ...
    if planet.eccentricity == None or planet.semi_major_radius == None:
        ...
        ecc_list.append(shortest_distance)
        ecc_list.append(longest_distance)
        
    return sorted(ecc_list)
...
for planet_info in all_planets_list:
    if planet_info.planet_name == 'b Cen AB b':
        distances_to_star_b_cen_ab_b = get_distances_to_star(planet_info)
...
def get_liquid_water_distances(planet):
    ...
    star_lum = stars_dict[planet.host_name].stellar_luminosity
    
    if star_lum == None:
        return None
    
    else:
        lq_wtr_short = math.sqrt(10 ** star_lum / 1.15)
        lq_wtr_longest = math.sqrt(10 ** star_lum / 0.53)
        
        water_dist_list.append(lq_wtr_short)
        water_dist_list.append(lq_wtr_longest)
        
    return sorted(water_dist_list)
...
for planet_info in all_planets_list:
    if planet_info.planet_name == 'Kepler-197 e':
        liquid_water_distances_kepler_197_e = get_liquid_water_distances(planet_info)
...
for planet_info in all_planets_list:
    ...
    star_distance = get_distances_to_star(planet_info)
    water_distance = get_liquid_water_distances(planet_info)
    
    if star_distance == None or water_distance == None:
        continue
        
    if star_distance[0] >= water_distance[0] and star_distance[0] <= water_distance[1] \
    and star_distance[1] >= water_distance[0] and star_distance[1] <= water_distance[1]:
        planets_with_liquid_water.append(planet_info.planet_name)
...
def get_surface_temperatures(planet):
    ...
    if planet.equilibrium_temperature == None:
        ...
        max_temp = ((1 - 0.0) ** (1/4)) * planet.equilibrium_temperature
        surf_temp.append(min_temp)
        surf_temp.append(max_temp)
        
    return sorted(surf_temp)
...
for planet_info in all_planets_list:
    if planet_info.planet_name == "HD 20794 d":
        surface_temp_hd_20794_d = get_surface_temperatures(planet_info)
...
pleasant_planets = []
for planet_info in all_planets_list:
    surface_temp_plan = get_surface_temperatures(planet_info)
    
    if surface_temp_plan == None:
        continue
    
    if surface_temp_plan[0] > 263 and surface_temp_plan[1] < 323:
        pleasant_planets.append(planet_info.planet_name)
...
for planet_info in all_planets_list:
    planet_grav = get_surface_gravity(planet_info)
    star_distance = get_distances_to_star(planet_info)
    water_distance = get_liquid_water_distances(planet_info)
    surface_temp = get_surface_temperatures(planet_info)
    
    if planet_grav == None or star_distance == None or water_distance == None or surface_temp == None:
        continue
        
    if planet_grav > 0.75 and planet_grav < 1.25:
       
        if star_distance[0] >= water_distance[0] and star_distance[0] <= water_distance[1] \
        and star_distance[1] >= water_distance[0] and star_distance[1] <= water_distance[1]:
            
            if surface_temp[0] > 200 and surface_temp[1] < 350:
                habitable_planets.append(planet_info.planet_name)
